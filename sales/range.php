...
<h2>
<?php echo "Sales From ".date('d-m-Y',strtotime($from))." to ".date('d-m-Y',strtotime($to)) ; ?></h2>
...
try
{
	...
	//select data from db
	$stmt = $conn->prepare("SELECT EMPID,PRODUCTID,NAME,COLORS,SIZE,
												SLEEVE,FLOWERSEMB,QUANTITY,PRICE,REVENUE,
												DATE_FORMAT(DATE(DATE_TIME),'%d-%m-%Y') AS DISDATE 
												FROM SALES 
