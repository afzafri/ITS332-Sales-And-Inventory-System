...
<h1 class='page-header'>Update Stocks</h1>

<form action="upstock.php" method="post">

<label><select name='pilih' onchange="fetch_data(this.value);">
...
try
{
	...
	<input type='submit' name='update' class='myButton' value='Update Stock'>
	...
	if(isset($_POST['update']))
	{
		...
		{
			//insert data into db
			$stmt = $conn->prepare("UPDATE PRODUCTDETAIL 
														SET QUANTITY='{$bilangan}' 
														WHERE PRODUCTID='{$idproduk}' AND 
														SIZE='{$saiz}' AND 
														COLORS='{$warna}' AND 
														SLEEVE='{$lengan}' AND 
														FLOWERSEMB='{$bunga}' ");
			...
			echo "<script>alert('Success! Stock Updated.')</script>";
		}
		if($bilangan <= 0 && $idproduk != null && $saiz != null && $warna != null)
		{
